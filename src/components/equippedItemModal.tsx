...
function createEmptyItem(slot: EquipmentSlot): EquippedItem {
  return {
    id: createEquipmentId(),
    slot,
    category: getDefaultEquipmentCategory(slot),
    name: "",
    description: "",
    isTwoHanded: false,
    effects: [],
...
export function EquippedItemModal({
  ...
  const handleSave = () => {
    ...
    onSave({
      ...draft,
      slot,
      category: allowedCategories.includes(draft.category)
        ? draft.category
        : allowedCategories[0],
  ...
  return createPortal(
    ...
    >
      ...
      <div
        className="gold-border parchment-bg relative z-10 w-full max-w-4xl rounded-[30px] p-6 md:p-7"
